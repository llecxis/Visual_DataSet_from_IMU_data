...
class vtpDrawScene: 
    ...
    def SetRefQuatOrientation_arrowshow( self, quaternion, shift, i_actor, motion_flag, ti, qtr_data ): #function to rotate all elements around origine
        if not self.iniOk_qt :
            raise Exception("vtpDrawScene not initialized. Call initScene() first")
        ...
        if (motion_flag[0] == 1):
            ...
            position = np.array(self.modelActorsArrows[i_actor].GetPosition()) #use if we need rotation in local FOR
            trans = position            
            ...
            # self.pos_set[i_actor][ti] = position + shift
        ...
        temp = tr.GetMatrix()
        temp = list(self.initial_pos_actorsArrows[i_actor])
        temp.append(0.0)

        depth_der = [512,424]
        color_der = [1920,1080]
        temp_pos = tr.GetMatrix().MultiplyPoint(temp)
        temp_depth = [temp_pos[0]*depth_der[0]/2 + depth_der[0]/2, temp_pos[2]*depth_der[1] + depth_der[1]/2]
        temp_color = [temp_pos[0]*color_der[0]/2 + color_der[0]/2, temp_pos[2]*color_der[1] + color_der[1]/2]
        temp_orientation = qtr_data
        
        trackingState = 0
        self.pos_set[i_actor][ti] = list(np.hstack((temp_pos,temp_depth,temp_color,temp_orientation)))
        # self.pos_set[i_actor][ti] = np.array(self.modelActorsArrows[i_actor].GetPosition())
        self.modelActorsArrows[i_actor].SetUserTransform(tr)
    ...
    def init_pos_actor( self, i_actor, mov):
        ...
        real_pos = np.array(self.modelActorsArrows[i_actor].GetPosition())
        new_pos = real_pos + mov
        self.modelActorsArrows[i_actor].SetPosition(new_pos)
