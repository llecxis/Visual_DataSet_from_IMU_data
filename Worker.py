...
class Worker(QObject):
    ...
    sig_shifts = pyqtSignal(str, list)  # worker id, step description: emitted every 5 steps through work() loop
    sig_qtr = pyqtSignal(str, list)  # worker id, step description: emitted every step through work() loop
    sig_status = pyqtSignal(str, int)  # worker id: emitted status()
